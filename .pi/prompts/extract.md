...
This is a **design** activity, not a cleanup. The goal is not to find "duplication" and mechanically remove it. The goal is to ask: _what better structure could exist here?_
...
**Survey first, propose second.** Read broadly across the area — not just the files that look duplicated, but files that _should_ share a pattern but don't. Build a mental model of the shapes that repeat, and the shapes that are weird one-offs.
...
If the user wants to proceed, say _"Want me to /plan this?"_ — do not implement directly from this prompt. The plan step is the gate between design and code.
