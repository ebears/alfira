...
**What `bun setup:nodelink` does:** Clones the [NodeLink](https://github.com/PerformanC/NodeLink) audio server at the pinned commit into `.nodelink/`, installs its dependencies, builds it, and copies the project's custom config (`nodelink.config.ts`) into place. The pin lives in `.nodelink-version` (the same file the Dockerfile uses). Dependencies are installed from a vendored lockfile (`.nodelink-bun.lock`), so every install resolves to exactly the versions the pin was verified against — the setup also disables NodeLink's startup dependency auto-updater, which would otherwise run `bun add` and mutate `node_modules` on every boot. The resulting `.nodelink/` directory is excluded from git. Re-running the script is safe — it moves the checkout to the pinned commit and rebuilds, which is how you pick up a NodeLink version bump. When bumping the pin in `.nodelink-version`, regenerate the lockfile too: `rm -rf .nodelink && bun setup:nodelink && cp .nodelink/bun.lock .nodelink-bun.lock`. NodeLink is spawned automatically as a child process when the dev server starts.
