...
const ACCESS_TOKEN_MAX_AGE_S = 60 * 60; // 1 hour in seconds
...
const REFRESH_TOKEN_MAX_AGE_S = Math.floor(REFRESH_TOKEN_MAX_AGE_MS / 1000);

// ---------------------------------------------------------------------------
// Cookie helpers — operate on Elysia's reactive cookie proxy.
// Elysia's cookie is typed as Record<string, Cookie<unknown>>.
// Setting .value, .httpOnly, .secure etc. automatically sets Set-Cookie.
// ---------------------------------------------------------------------------
...
function setAuthCookies(
  ...
  const session = cookie[ACCESS_COOKIE_NAME];
  if (session) {
  ...
  if (refresh) {
    ...
    refresh.path = '/';
    refresh.maxAge = REFRESH_TOKEN_MAX_AGE_S;
  }
}

function clearAuthCookies(cookie: Record<string, { remove?: () => void }>): void {
  cookie[ACCESS_COOKIE_NAME]?.remove?.();
  cookie[REFRESH_COOKIE_NAME]?.remove?.();
}
...
async function generateAndStoreTokens(
  ...
  const refreshTokenExpiry = new Date(Date.now() + REFRESH_TOKEN_MAX_AGE_MS).toISOString();
...
export const authPlugin = new Elysia({ name: 'auth' })
  ...
  // ── /auth/callback ── exchange code, set cookies, redirect
  .get('/callback', async ({ redirect, cookie, request }) => {
    ...
    if (!setupDone) {
      ...
      if (envGuildId) {
        ...
        // Fresh install — set tokens and redirect to setup wizard.
    ...
    // 7. Set cookies and redirect home.
    setAuthCookies(cookie, accessToken, refreshToken);
