...
// ---------------------------------------------------------------------------
// Handlers — GET /playlists
// ---------------------------------------------------------------------------

async function handleGetPlaylists(ctx: Record<string, unknown>): Promise<Response> {
...
// ---------------------------------------------------------------------------
// Handlers — POST /playlists
// ---------------------------------------------------------------------------

async function handlePostPlaylist(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — GET /playlists/:id
// ---------------------------------------------------------------------------

async function handleGetPlaylist(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — PATCH /playlists/:id/visibility
// ---------------------------------------------------------------------------

async function handlePatchVisibility(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — PATCH /playlists/:id
// ---------------------------------------------------------------------------

async function handlePatchPlaylist(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — DELETE /playlists/:id
// ---------------------------------------------------------------------------

async function handleDeletePlaylist(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — POST /playlists/:id/songs
// ---------------------------------------------------------------------------

async function handleAddSong(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — DELETE /playlists/:id/songs/:songId
// ---------------------------------------------------------------------------

async function handleRemoveSong(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — POST /playlists/:id/songs/bulk-remove
// ---------------------------------------------------------------------------

async function handleBulkRemoveSongs(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
// ---------------------------------------------------------------------------
// Handlers — PATCH /playlists/:id/reorder
// ---------------------------------------------------------------------------

async function handleReorderSongs(ctx: Record<string, unknown>): Promise<Response> {
  const authErr = requireAuth({ user: ctx.user as never, isAdmin: ctx.isAdmin as boolean });
  if (authErr) {
    return authErr;
  }
...
export function playlistsPlugin(app: Elysia): Elysia {
  return app
    .get('/playlists', handleGetPlaylists as never)
    .post('/playlists', handlePostPlaylist as never, {
      body: PlaylistCreateSchema,
    })
    .post('/playlists/:id/songs/bulk-remove', handleBulkRemoveSongs as never, {
      body: PlaylistRemoveSongsSchema,
    })
    .delete('/playlists/:id/songs/:songId', handleRemoveSong as never)
    .post('/playlists/:id/songs', handleAddSong as never, {
      body: PlaylistAddSongSchema,
    })
    .patch('/playlists/:id/visibility', handlePatchVisibility as never, {
      body: PlaylistVisibilitySchema,
    })
    .get('/playlists/:id', handleGetPlaylist as never)
    .patch('/playlists/:id', handlePatchPlaylist as never, {
      body: PlaylistCreateSchema,
    })
    .patch('/playlists/:id/reorder', handleReorderSongs as never, {
      body: PlaylistReorderSchema,
    })
    .delete('/playlists/:id', handleDeletePlaylist as never) as unknown as Elysia;
}
